...
const AIInsightsDashboard = () => {
    ...
    const [realtimeEvents, setRealtimeEvents] = (0, react_1.useState)([]);
    ...
    const initializeSocket = () => {
        const newSocket = (0, socket_io_client_1.io)('ws://localhost:3000');
    ...
    const handleInsightAction = (insight, actionIndex) => {
        ...
        setInsights(prev => prev.map(i => i.id === insight.id
            ? { ...i, data: { ...i.data, actionTaken: action } }
            : i));
    };
    const filteredInsights = insights.filter(insight => activeFilter === 'all' || insight.severity === activeFilter);
    ...
    return (<div className="bg-white rounded-lg shadow-sm">
      ...
      <div className="border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <div className={`flex items-center space-x-2 px-3 py-1 rounded-full text-sm ${isConnected ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
        ...
        <div className="flex space-x-1 mt-4">
          {['all', 'critical', 'warning', 'info'].map(filter => (<button key={filter} onClick={() => setActiveFilter(filter)} className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${activeFilter === filter
                ? 'bg-blue-100 text-blue-700 border border-blue-200'
                : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'}`}>
              {filter.charAt(0).toUpperCase() + filter.slice(1)}
              {filter !== 'all' && (<span className="ml-2 px-2 py-1 text-xs bg-gray-200 text-gray-700 rounded-full">
      ...
      <div className="flex">
        ...
        <div className="flex-1 p-6">
          <div className="space-y-4">
            {filteredInsights.map((insight) => (<div key={insight.id} className={`border rounded-lg p-4 cursor-pointer transition-all hover:shadow-md ${selectedInsight?.id === insight.id ? 'ring-2 ring-blue-500' : ''} ${getSeverityColor(insight.severity)}`} onClick={() => setSelectedInsight(insight)}>
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    {insight.actionable && insight.recommendations.length > 0 && (<div className="mt-3 flex flex-wrap gap-2">
                        {insight.recommendations.slice(0, 2).map((rec, index) => (<button key={index} onClick={(e) => {
                        e.stopPropagation();
                        handleInsightAction(insight, index);
                    }} className="text-xs bg-blue-600 text-white px-3 py-1 rounded-full hover:bg-blue-700 transition-colors">
                            {rec.length > 40 ? rec.substring(0, 40) + '...' : rec}
                          </button>))}
                        {insight.recommendations.length > 2 && (<span className="text-xs text-gray-600 px-2 py-1">
            ...
            {filteredInsights.length === 0 && (<div className="text-center py-12">
        ...
        <div className="w-80 border-l border-gray-200 bg-gray-50">
          {selectedInsight ? (<div className="p-6">
              ...
              <div className="space-y-4">
                ...
                {selectedInsight.recommendations.length > 0 && (<div>
                    <h4 className="font-medium text-gray-900 mb-2">Recommended Actions</h4>
                    <div className="space-y-2">
                      {selectedInsight.recommendations.map((rec, index) => (<div key={index} className="flex items-start space-x-2">
                          <div className="text-blue-600 text-sm mt-1">•</div>
                          <div className="flex-1">
                            <p className="text-sm text-gray-700">{rec}</p>
                            <button onClick={() => handleInsightAction(selectedInsight, index)} className="text-xs text-blue-600 hover:text-blue-800 mt-1">
                              Execute Action →
                            </button>
                          </div>
                        </div>))}
      ...
      <div className="border-t border-gray-200 bg-gray-50 px-6 py-4">
        ...
        <div className="space-y-2 max-h-32 overflow-y-auto">
          {realtimeEvents.slice(0, 5).map((event, index) => (<div key={index} className="flex items-center space-x-3 text-sm">
          ...
          {realtimeEvents.length === 0 && (<p className="text-sm text-gray-500 italic">Waiting for real-time events...</p>)}
        </div>
      </div>
    </div>);
};
exports.default = AIInsightsDashboard;
//# sourceMappingURL=AIInsightsDashboard.js.map