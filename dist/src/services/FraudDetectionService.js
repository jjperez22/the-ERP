...
let FraudDetectionService = class FraudDetectionService {
    ...
    async checkTransactionLocation(transaction) {
        const indicators = [];
        ...
        const recentTransactions = await this.getRecentTransactions(transaction.userId, new Date(Date.now() - 2 * 60 * 60 * 1000));
        for (const recentTx of recentTransactions) {
            if (recentTx.location && transaction.location) {
                const distance = this.calculateDistance({ lat: transaction.location.coordinates.latitude, lon: transaction.location.coordinates.longitude }, { lat: recentTx.location.coordinates.latitude, lon: recentTx.location.coordinates.longitude });
                const timeElapsed = (transaction.timestamp.getTime() - new Date(recentTx.timestamp).getTime()) / (1000 * 60 * 60);
                const maxPossibleSpeed = distance / timeElapsed;
    ...
    async checkTransactionPatterns(transaction) {
        const indicators = [];
        ...
        const recentEvents = await this.getRecentSecurityEvents(transaction.userId, 30 * 60 * 1000);
    ...
    async checkMerchantReputation(transaction) {
        const indicators = [];
    ...
    async checkTransactionTime(transaction) {
        const indicators = [];
        const day = transaction.timestamp.getDay();
        const hour = transaction.timestamp.getHours();
        if ((day === 0 || day === 6) && transaction.amount > 5000) {
            indicators.push({
                type: 'pattern',
    ...
    async getRecentSecurityEvents(userId, timeWindowMs) {
        try {
    ...
    async getMerchantTransactionHistory(merchant) {
        try {
            return await this.databaseService.find('security_events', {
    ...
    async recordFraudulentTransaction(transaction, riskScore, indicators) {
        try {
            const fraudRecord = {
                id: this.generateId(),
